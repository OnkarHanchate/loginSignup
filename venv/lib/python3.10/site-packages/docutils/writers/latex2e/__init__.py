...
class Writer(writers.Writer):
    ...
    table_style_values = [  # TODO: align-left, align-center, align-right, ??
                          'usertabs', 'borderless', 'colwidths-auto',
                          'nolines', 'standard']
    ...
    settings_spec = (
        ...
        (('Specify LaTeX documentclass.  Default: "article".',
         ...
         ('Table style. "standard" with horizontal and vertical lines, '
          '"usertabs" (LaTeX usertabs style) only horizontal lines '
...
class CharMaps:
    ...
    # http://aspn.activestate.com/ASPN/Cookuser/Python/Recipe/252124
...
class DocumentClass:
    ...
    def __init__(self, document_class, with_part=False):
        ...
        if self.document_class in ('user', 'memoir', 'report',
                                   'scruser', 'scrreprt'):
...
class Table:
    ...
    :usertabs:   only horizontal lines (requires "usertabs" LaTeX package)
    ...
    def set_table_style(self, node, settings):
        ...
        borders = [cls.replace('nolines', 'borderless')
                   for cls in (['standard']
                               ...
                               + node['classes'])
                   if cls in ('standard', 'usertabs', 'borderless', 'nolines')]
...
class LaTeXTranslator(nodes.NodeVisitor):
    ...
    def __init__(self, document, babel_class=Babel):
        ...
        # LaTeX Toc
        # include all supported sections in toc and PDF usermarks
    ...
    def visit_table(self, node):
        ...
        if self.active_table.borders == 'usertabs':
            self.requirements['usertabs'] = r'\usepackage{usertabs}'
        self.push_output_collector([])
    ...
    def visit_title(self, node):
        ...
        # Section title
            ...
            # label and ToC entry:
            usermark = ['']
            # add sections with unsupported level to toc and pdfusermarks?
            ## if level > len(self.d_class.sections):
            ##     section_title = self.encode(node.astext())
            ##     usermark.append(r'\addcontentsline{toc}{%s}{%s}' %
            ##               (section_name, section_title))
            usermark += self.ids_to_labels(node.parent, set_anchor=False)
            self.context.append('%\n  '.join(usermark) + '%\n}\n')
            if (level > len(self.d_class.sections)
    ...
    def visit_contents(self, node):
        ...
        # labels and PDF usermark (sidebar entry)
        self.out.append('\n')  # start new paragraph
        ...
        if (isinstance(node.next_node(), nodes.title)
            ...
            self.out.append('\\pdfusermark[%d]{%s}{%s}\n' %
                            (self.section_level+1,
